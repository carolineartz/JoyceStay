...
class Listing extends Component {
  ...
  componentDidMount() {
    const { fetchListing, fetchListingReviews } = this.props;
  ...
  render() {
    ...
    const canLeaveReview = Boolean(
      _.find(user.listing_booking_ids, { listing_id: id, status: "APPROVED" })
    );
        ...
        <section className="content-container--interior-page flex-container">
          <section className="listing-details-container grid--75">
            <div className="listing-details__header">
              <div>
                {Object.values(home_types)
                  .filter((ht) => ht.id == home_type_id)
                  .map((ht) => (
                    <h6 key={ht.id} className="text--maroon">
                      {ht.name}
                    </h6>
                  ))}
                <h2>
              ...
              <div className="profile-thumb-wrapper">
                <Link to={`/users/${user_id}/profile`}>
                  <div
                    ...
                    style={{ backgroundImage: `url(${ownerPhotoUrl})` }}
                  ></div>
                </Link>
            ...
            <div className="flex-container--no-justify rating-container">
              {review_ids.length ? (
                <>
                  <h3>
                    {review_ids.length} Review{review_ids.length > 1 ? "s" : ""}
                  </h3>
                  <br />
                  <Rating
          ...
          <StickyBox offsetTop={80}>
            <ListingSidebar
              listing={listing}
              checkBlockedDays={this.checkBlockedDays}
            />
          </StickyBox>
        </section>
      </>
